...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('BYY STORE', {
		font: 'Standard',
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6289507948843@s.whatsapp.net"
...
kurr.on("group-update", async (anu) => {
    ...
    if (anu.announce == "false") {
      ...
      teks = `- [ OPEN ✅ ] -\n\n_*GRUP SUDAH BUKA GAS ORDERR*\n*GAS ORDER YOK GAES*`;

      kurr.sendMessage(metdata.id, teks, MessageType.text);

      console.log(`- [ OPEN ✅ ] - In ${metdata.subject}`);

    } else if (anu.announce == "true") {

      teks = `- [ CLOSE ❎ ] -\n\n*GRUP SUDAH TUTUP YAK*\n*TERIMAKASIH YANG SUDAH ORDERR*`;

      kurr.sendMessage(metdata.id, teks, MessageType.text);

      console.log(`- [ CLOSE ✅ ] - In ${metdata.subject}`);
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `Byy Store`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6289507948843@s.whatsapp.net","6281220670449@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6289507048843@s.whatsapp.net"
